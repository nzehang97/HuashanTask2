...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        self.name = os.path.splitext(os.path.basename(path))[0]  # 将path拆分为[‘文件名’，‘拓展名’]  C3L-00081-21
        self.wsi = openslide.open_slide(path)  # 读取图像 OpenSlide('DATA_DIRECTORY\\C3L-00081-21.svs')
        self.level_downsamples = self._assertLevelDownsamples() # [(1.0,1.0),(4.0,4.0),(16.0,16.0)]
        self.level_dim = self.wsi.level_dimensions  # 原始图像经各个level水平缩放后的长和宽 ((25895, 24645), (6473, 6161), (1618, 1540))
    
        self.contours_tissue = None  # 组织轮廓
        self.contours_tumor = None  # 肿瘤轮廓
        self.hdf5_file = None

    def getOpenSlide(self):  # 读取图像
        return self.wsi

    def initXML(self, xml_path):
        def _createContour(coord_list):  # 私有方法
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up = 255, mthresh=7, close = 0, use_otsu=False, 
                            filter_params={'a_t':100}, ref_patch_size=256, exclude_ids=[], keep_ids=[]):
        """
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            ...
            hierarchy_1 = np.flatnonzero(hierarchy[:,1] == -1)  # 返回满足条件的位置
            all_holes = []
        ...
        img = np.array(self.wsi.read_region((0,0), seg_level, self.level_dim[seg_level]))  # 读取指定区域
        ...
        contours, hierarchy1 = cv2.findContours(img_otsu, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE) # Find contours
        hierarchy = np.squeeze(hierarchy1, axis=(0,))[:, 2:]
    ...
    def process_contours(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        ...
        fp_chunk_size = math.ceil(n_contours * 0.05)  # 将 x 向上舍入到最接近的整数
        init = True
        for idx, cont in enumerate(self.contours_tissue):
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        x_coords, y_coords = np.meshgrid(x_range, y_range, indexing='ij')  # 生成patch的网格坐标
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        if vis_level < 0:
            vis_level = self.wsi.get_best_level_for_downsample(scales)
